...
namespace CarPricer
{
    ...
    public class PriceDeterminator
    {
        ...
        {
            if ((car.AgeInMonths / 12) <= 10)
            {
                car.PurchaseValue -= car.PurchaseValue * (car.AgeInMonths * 0.005m);
            }

            decimal milesDesc = (Math.Min(150000, car.NumberOfMiles) / 1000) * 0.002m;
            car.PurchaseValue -= car.PurchaseValue * milesDesc;


            if (car.NumberOfPreviousOwners > 2)
            {
                car.PurchaseValue -= car.PurchaseValue * 0.25m;
            }

            car.PurchaseValue -= car.PurchaseValue * (car.NumberOfCollisions * 0.02m);

            if (car.NumberOfPreviousOwners == 0)
            {
                car.PurchaseValue += car.PurchaseValue * 0.1m;
            }
    ...
    public class UnitTests
    {
        ...
        public static void CalculateCarValue()
        {
